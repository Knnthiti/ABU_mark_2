...
float SCALE_FACTOR_Gear_1 = 0.095f;
...
void setup() {
  ...
  // robot.Setup_max_speed(0.0, 190.0, 210.0, 225.0);
  ...
  robot.Shoot_Ball_Setup(pin_Shoot, Attack_Ball, Reload_Ball);
}
...
void loop() {
  ...
  if (robot.seting[1] == 1) {
    ...
    if (robot.Ready == 1) {
      robot.UP_speed(button_speedDOWN, button_speedUP);
      robot.Move();
